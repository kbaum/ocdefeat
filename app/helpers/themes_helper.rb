module ThemesHelper
  def tally_popularity(theme)
    if theme.obsessions_per_theme == 0
      ...
      <<-HEREDOC
      #{theme.prevalence_in_patients} distinct #{'patient'.pluralize(theme.prevalence_in_patients)}
      #{'is'.pluralize(theme.prevalence_in_patients)} currently obsessing about #{theme.name}, in which
