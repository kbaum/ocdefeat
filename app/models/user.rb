class User < ApplicationRecord
  ...
  scope :patients_obsessive_but_symptomless, -> { patients_obsessing.where.not("exists ( #{Obsession.presenting_symptoms.where('users.id = obsessions.user_id').select(1).to_sql} )") }
